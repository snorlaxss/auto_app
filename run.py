...
try:
    ...
    HAS_JOINT_STATE_MSG = True
except ImportError:
    print("Warning: sensor_msgs JointState not found. WebSocket joint streaming will use zero fallback.")
    JointState = None
    HAS_JOINT_STATE_MSG = False
...
try:
    ...
    from robotaction.common import TaskStatus, normalize_state_text
...
class ROS2ImageSubscriber(Node):
    def __init__(self):
        ...
        self.joint_state_topic = os.getenv("MARVIN_JOINT_STATE_TOPIC", "/joint_states")
        self.joint_state_lock = threading.Lock()
        self.latest_joint_state = {}
        self.latest_joint_state_updated_at = 0.0
        self.joint_state_sub = None
        if HAS_JOINT_STATE_MSG:
    ...
    def joint_state_callback(self, msg):
        ...
        with self.joint_state_lock:
            self.latest_joint_state.update(snapshot)
            self.latest_joint_state_updated_at = now

    def build_joint_state_message(self, seq, robot_state="idle"):
        del robot_state
        now = time.time()
        with self.joint_state_lock:
            latest = copy.deepcopy(self.latest_joint_state)
            updated_at = float(self.latest_joint_state_updated_at or 0.0)
        is_stale = updated_at <= 0.0 or (now - updated_at) > 1.0
        joints = []
...
class RobotApiService:
    def __init__(self):
        ...
        self.default_marvin_object_yaml = _default_marvin_object_yaml()
        self.default_marvin_status_json = _default_marvin_status_json()
        self.final_state_id, self.final_state_description = self._load_final_state(
            self.default_marvin_status_json
        )
        self.state = {
            "sam3_segmenter": None,
            "sam3_detections": [],
    ...
    def _load_final_state(self, status_json_path):
        try:
            resolved_path = _resolve_marvin_path(status_json_path)
            with open(resolved_path, "r", encoding="utf-8") as f:
                data = json.load(f) or {}
            nodes = data.get("nodes", [])
            for node in reversed(nodes):
                description = normalize_state_text(node.get("state_description"))
                if description:
                    return str(node.get("node_id", "")).strip(), description
        except Exception as exc:
            print(f"[API] failed to load final state from {status_json_path}: {exc}")
        return "", ""

    def _state_matches_final(self, stable_state):
        stable = normalize_state_text(stable_state)
        final_state = normalize_state_text(self.final_state_description)
        return bool(stable and final_state and stable == final_state)
    ...
    def startup(self):
        ...
        if _env_bool("API_INIT_MODELS", True):
            try:
                ...
                print(f"[API] model initialization failed: {exc}")

        set_fresh_tf_callback(self.fresh_tf_capture)
    ...
    def init_models(self):
        ...
        if self.state["sam3_segmenter"] is None:
            self.state["sam3_segmenter"] = initialize_sam3(device)

        if self.state["flowpose_estimator"] is None:
            self.state["flowpose_estimator"] = FlowPoseEstimator(device)

        try:
            siglip_worker, _siglip_status = start_siglip_realtime_publisher(device)
            self.state["siglip_worker"] = siglip_worker
        except Exception as exc:
            self.state["siglip_worker"] = None
            print(f"[API] Siglip realtime publisher failed: {exc}")

        return {
            "device": str(device),
            "sam3_ready": self.state["sam3_segmenter"] is not None,
            "flowpose_ready": self.state["flowpose_estimator"] is not None,
            "siglip_running": bool(self.state["siglip_worker"] is not None),
            "ros2_running": bool(rclpy.ok()),
        }

    def ensure_models_ready(self):
        if self.state["sam3_segmenter"] is not None and self.state["flowpose_estimator"] is not None:
            return {
                "device": str(self.state.get("device") or ""),
                "sam3_ready": True,
                "flowpose_ready": True,
            }
        return self.init_models()

    def _reset_frame_state(self, color_image, depth_image, depth_scale):
        self.state["color_image"] = color_image
        self.state["depth_image"] = depth_image
        self.state["depth_scale"] = depth_scale
        self.state["masks"] = None
        self.state["labels"] = None
        self.state["sam3_detections"] = []
        self.state["pose_results"] = None
        self.state["length_results"] = None
        self.state["data_results"] = None
    ...
    def _capture_perception_impl(self, prompts_text, suppress_contained_masks):
        ...
        self._reset_frame_state(color_image, depth_image, depth_scale)
        ...
        self.state["masks"] = masks if masks else None
        self.state["labels"] = labels if labels else None
        self.state["sam3_detections"] = detections
        segment_stats = getattr(self.state["sam3_segmenter"], "last_segment_stats", {}) or {}
        for label in labels:
            if label not in self.state["label_history"]:
                self.state["label_history"].append(label)
        ...
        if combined_mask is None or not obj_ids:
            image_rgb = cv2.cvtColor(mask_vis, cv2.COLOR_BGR2RGB)
            return {
                "image_rgb": image_rgb,
                "labels": list(labels),
        ...
        self.state["length_results"] = length_all
        self.state["data_results"] = flowpose_result

        if pose_all is not None and _ros2_node is not None:
            _ros2_node.publish_poses_as_tf(np.asarray(pose_all, dtype=np.float32), labels)
    ...
    def fresh_tf_capture(self, reason="", target=None):
        prompts_text = os.getenv("FRESH_TF_PROMPTS", "").strip() or self.default_sam3_prompts
        if os.getenv("FRESH_TF_PROMPT_MODE", "default").strip().lower() == "target" and target:
            values = target if isinstance(target, (list, tuple, set)) else [target]
            prompts = []
            for value in values:
                text = str(value or "").strip().lower()
                text = re.sub(r"_\d+$", "", text).replace("_", " ")
        ...
        try:
            result = self.capture_perception(prompts_text, suppress_contained)
            return result.get("status") in {"ok", "no_detection", "mask_empty"}, result.get("message", "success")
        except Exception as exc:
    ...
    def encode_capture(self, capture_result, request):
        ...
        if fmt in {"jpg", "jpeg"}:
            fmt = "jpeg"
            ext = ".jpg"
            params = [int(cv2.IMWRITE_JPEG_QUALITY), int(request.quality)]
        elif fmt == "png":
            ext = ".png"
            params = []
        else:
    ...
    def capture(self, request):
        result = self.capture_perception()
        return self.encode_capture(result, request)

    def _has_running_operation(self):
        return any(op.get("status") == "running" for op in self.operations.values())

    def _running_grasp_operation_locked(self):
    ...
    def _current_stable_state(self):
        node = _robotaction_node
        if node is None:
            return None
        watcher = getattr(node, "status_watcher", None)
        if watcher is None:
            return None
        try:
            return normalize_state_text(watcher.get_stable_state())
        except Exception:
            return None

    def _refresh_operation_completion_locked(self):
        stable_state = self._current_stable_state()
        now = time.time()
        reached_final = self._state_matches_final(stable_state)
        for op in self.operations.values():
            if op.get("operation_type") != "grasp":
                continue
            if reached_final:
                if op.get("status") != "completed" or op.get("result") != "success":
                    op["updated_at"] = now
                    op["duration_ms"] = int((now - float(op.get("created_at", now))) * 1000)
                op["status"] = "completed"
                op["result"] = "success"
                op["completed_by_state"] = stable_state
                op["final_state_id"] = self.final_state_id
            else:
                if op.get("status") == "completed":
                    op["updated_at"] = now
                    op["state_left_final_at"] = now
                op["status"] = "running"
                op.pop("result", None)
                op.pop("completed_by_state", None)
        return stable_state
    ...
    def start_grasp(self, request):
        ...
        with self.operation_lock:
            self._refresh_operation_completion_locked()
            running_grasp = self._running_grasp_operation_locked()
            if running_grasp is not None:
                node = _robotaction_node
                if node is not None and getattr(node, "_fresh_tf_paused", False):
                    try:
                        resumed_node, message = start_marvin_action_node(
                            object_yaml_path=self.default_marvin_object_yaml,
                            status_json_path=self.default_marvin_status_json,
                            status_topic=_default_marvin_config_value("status_topic", "MARVIN_STATUS_TOPIC", "/siglip/result"),
                            progress_topic=_default_marvin_config_value(
                                "progress_topic",
                                "MARVIN_PROGRESS_TOPIC",
                                "/control/task_percentage",
                            ),
                        ...
                        self.state["marvin_action_node"] = resumed_node
                    except Exception as exc:
                        raise ApiError(1003, f"机器人未连接 / Marvin恢复失败: {exc}", 409)

                    running_grasp["updated_at"] = time.time()
                    running_grasp["pause_requested"] = False
                    running_grasp["resumed_at"] = running_grasp["updated_at"]
                    running_grasp["resume_message"] = message
                    running_grasp["resume_count"] = int(running_grasp.get("resume_count", 0) or 0) + 1
                    return {
                        "operation_id": running_grasp["operation_id"],
                        "estimated_duration_ms": 4000,
                    }
                raise ApiError(1002, "机器人正在执行操作中，请先停止或等待完成", 409)

            try:
                node, message = start_marvin_action_node(
                    object_yaml_path=self.default_marvin_object_yaml,
                    status_json_path=self.default_marvin_status_json,
                    status_topic=_default_marvin_config_value("status_topic", "MARVIN_STATUS_TOPIC", "/siglip/result"),
                    progress_topic=_default_marvin_config_value(
                ...
                self.state["marvin_action_node"] = node
            except Exception as exc:
                raise ApiError(1003, f"机器人未连接 / Marvin启动失败: {exc}", 409)
            ...
            op = self._store_operation(
                ...
                completion_condition={
                    ...
                    "final_state_description": self.final_state_description,
                },
            )
            return {
    ...
    def stop(self, request):
        ...
        with self.operation_lock:
            stopped = stop_marvin_action_node(publish_home=True)
            self.state["marvin_action_node"] = _robotaction_node
            now = time.time()
            for op in self.operations.values():
                if op.get("operation_type") == "grasp" and op.get("status") == "running":
                    op["pause_requested"] = True
                    op["paused_at"] = now
                    op["last_stop_mode"] = request.mode
                    op["last_stop_emergency"] = bool(request.emergency)
                    op["home_before_pause"] = True
                    op["updated_at"] = now
            ...
            return {
                "operation_id": stop_op["operation_id"],
                "mode": request.mode,
            }

    def get_operation(self, operation_id):
        with self.operation_lock:
            stable_state = self._refresh_operation_completion_locked()
            op = self.operations.get(operation_id)
            if not op:
                raise ApiError(1001, f"operation_id 不存在: {operation_id}", 404)
            result = dict(op)
            if result.get("operation_type") == "grasp":
                result["current_state"] = stable_state
                result["final_state_id"] = self.final_state_id
                result["final_state_description"] = self.final_state_description
            return result
    ...
    def status(self):
        ...
        if _ros2_node is None or not rclpy.ok():
            robot_state = "offline"
        elif node is not None and getattr(node, "_fresh_tf_paused", False):
            robot_state = "holding"
        elif self._has_running_operation():
            robot_state = "moving"
        else:
            robot_state = "idle"
        ...
        with self.operation_lock:
            for op in self.operations.values():
                if op.get("status") == "running":
                    current_operation = op.get("operation_id")
                    break

        camera_streaming = bool(_ros2_node is not None and getattr(_ros2_node, "rs_pipeline", None) is not None)
        camera_fps = int(getattr(_ros2_node, "rs_fps", 0) or 0) if _ros2_node is not None else 0
        return {
            "robot": {
                "connected": bool(_ros2_node is not None and rclpy.ok()),
                "state": robot_state,
    ...
    def joint_state_message(self, seq):
        running = self._has_running_operation()
        robot_state = "moving" if running else "idle"
        if _ros2_node is not None and hasattr(_ros2_node, "build_joint_state_message"):
            return _ros2_node.build_joint_state_message(seq, robot_state=robot_state)
...
def create_api_app():
    ...
    api = FastAPI(title="Robot Teleoperation API", version="1.0.0")
    ...
    def capture(request: CaptureRequest):
        try:
            data = service.capture(request)
            return _api_envelope(data=data)
        except Exception as exc:
    ...
    def grasp(request: GraspRequest):
        try:
            data = service.start_grasp(request)
            return _api_envelope(data=data, message="抓取操作已启动")
        except Exception as exc:
            return _api_error_response(exc)
    ...
    def stop(request: StopRequest):
        try:
            data = service.stop(request)
            return _api_envelope(data=data, message="停止指令已发送")
        except Exception as exc:
            return _api_error_response(exc)
...
def main():
    if _env_bool("AUTO_APP_USE_GRADIO", False):
        init_ros2()
