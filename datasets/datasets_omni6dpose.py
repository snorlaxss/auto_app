...
from utils.misc import get_pose_representation
        ...
        if self.per_obj:
            ...
            if os.path.exists(img_list_cache_filename):
                ...
                for i in tqdm(range(len(img_list))):
                    meta_path = img_list[i] + "meta.json"
                    try:
                        with open(meta_path, 'r') as f:
                            meta_json = json.load(f)
                    except Exception as e:
                        raise RuntimeError(f"Failed to read meta json '{meta_path}': {e}")
                    for obj_k, obj_v in meta_json.get('objects', {}).items():
                        if isinstance(obj_v, dict):
                            obj_v.pop('rotation', None)
                    gts = ImageMetaData(**meta_json)
    ...
    def __getitem__(self, index):
        if not self.cfg.load_per_object:
            ...
            if not any(obj.is_valid for obj in gts.objects):
                print(f'no valid object in {img_path}')
        ...
        # depth = Dataset.load_depth(img_path + ('depth_syn' if self.cfg.perfect_depth else 'depth') + '.exr') ## original code
        depth = Dataset.load_depth(img_path + ('depth_syn' if self.cfg.perfect_depth else 'depth') + '.exr')/1000.
        depth[depth > 1e3] = 0
        ...
        if inst_name not in self.obj_meta.instance_dict:
            print(f'instance {inst_name} not in obj_meta!')
            return self.__getitem__((index + 1) % self.__len__())
        ...
        if not np.any(object_mask):
            print(f'no valid object mask in {img_path}')
            # 打印mask的最大值和最小值
            print(f'mask max: {mask.max()}, mask min: {mask.min()}')
            print(f'mask shape: {mask.shape}, unique values: {np.unique(mask)[:10]}')  # 只显示前10个
            print(f'obj.mask_id: {obj.mask_id}, obj.meta.oid: {obj.meta.oid}')
            print(f'available mask_ids in image: {[o.mask_id for o in gts.objects if o.is_valid]}')
            return self.__getitem__((index + 1) % self.__len__())
        ...
        if np.sum(depth_valid) <= 1.0:
            print(f'no valid depth in {img_path}')
            return self.__getitem__((index + 1) % self.__len__())
        ...
        if len(pcl_in) < 50:
            print(f'no valid pcl in {img_path}')
            return self.__getitem__((index + 1) % self.__len__())
...
def process_batch(batch_sample, device, pose_mode='quat_wxyz', PTS_AUG_PARAMS=None):
    ...
    if 'old_sym_info' in batch_sample and PTS_AUG_PARAMS is not None:
        print("bruh")
    else:
    ...
    processed_sample = {
        'pts': PC_da,
        'pts_color': PC_da,
        'sym_info': batch_sample['sym_info'],
