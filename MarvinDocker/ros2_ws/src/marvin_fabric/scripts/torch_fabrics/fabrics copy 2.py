    ...
    def fabric_solve(self, pos: torch.Tensor, vel: torch.Tensor):
        ...
        for task_name in self.env.task_names:
            ...
            if task_space == "joint":
                ...
                # for frame_id_str in frames:
                p_list = []
                # indices = self.env.link_to_sphere_indices[frame_id_str]
                # link_sphere_ids, obs_sphere_ids = self.env.link_sphere_map[frame_id_str]
                ...
                for link_id, (link_sphere_ids, obs_sphere_ids) in self.env.link_sphere_map.items():
                    ...
                    # J_q1 = sphere_js[0][:3, :]
                    # frame_id = self.env.model.getFrameId(link_id)
                    # sphere_frame_id = self.env.sphere_to_frame_indices[link_sphere_ids[0]]
                    # J_1 = pin.getFrameJacobian(
                    #         self.env.model, self.env.data, sphere_frame_id, pin.ReferenceFrame.LOCAL_WORLD_ALIGNED
                    #     )[:3, :]
                    # J_2 = pin.getFrameJacobian(
                    #         self.env.model, self.env.data, frame_id, pin.ReferenceFrame.LOCAL_WORLD_ALIGNED
                    #     )[:3, :]
                    # J_q2 = link_j[frame_id][:3, :]
                    # diff_J = J_q1 - J_2
                    # diff2 = J_1 - J_q2
                    # print(np.linalg.norm(diff_J), " for link ", link_id, "sphere Jacobian and link Jacobian differ!")
                    # print(np.linalg.norm(diff2), " for link ", link_id, "sphere Jacobian and link Jacobian differ!")
                    # -> shape: (n_link, 3, nq)
                    ...
                    # link Jacobian wrt q: project each link’s geometric Jacobian into distance direction
                    # (n_link, n_obs, nq)
                    # J = torch.einsum('loj,lik->liko', J_x, J_q)  # WRONG shape originally
                    # Fix: contract diff-direction with correct axes
                    ...
                    # Second-order term: (vxᵀ (I/‖d‖ − ddᵀ/‖d‖³) vx) / (r_i + r_j)
                    # I = torch.eye(3, dtype=torch.float64, device=diff.device)
                    # diff_unit = diff / norms
