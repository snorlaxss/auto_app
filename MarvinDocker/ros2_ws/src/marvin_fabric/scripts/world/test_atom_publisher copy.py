...
class TestKeypointPublisher(Node):
    ...
    def timer_callback(self):
        ...
        # 模拟轨迹两个点
        pose1 = Pose()
        pose1.position = Point(x=0.4, y=0.0, z=0.1)
        pose1.orientation = Quaternion(x=0.0, y=0.0, z=0.0, w=1.0)
        pose2 = Pose()
        pose2.position = Point(x=0.4, y=0.0, z=-0.18)
        pose2.orientation = Quaternion(x=0.0, y=0.0, z=0.0, w=1.0)
        pose3 = Pose()
        pose3.position = Point(x=0.4, y=0.0, z=-0.2)
        pose3.orientation = Quaternion(x=0.0, y=0.0, z=0.0, w=1.0)
        pose4 = Pose()
        pose4.position = Point(x=0.5, y=0.0, z=-0.2)
        pose4.orientation = Quaternion(x=0.0, y=0.0, z=0.0, w=1.0)

        motion_kp.poses = [pose1, pose2, pose3, pose4]
        motion_kp.time = [0.0, 0.30, 0.5, 1.0]
        motion_kp.constraints = [100.0, 100.0, 100.0]
        motion_kp.speed = 0.5
        motion_kp.gripper_value = [0.0, 0.95, 0.8, 1.0]

        msg.poses = [home_kp, motion_kp]
