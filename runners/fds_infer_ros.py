...
class GenPose2Publisher(Node):
    def __init__(self):
        super().__init__('genpose2_publisher')
        self.depth_pub = self.create_publisher(Image, '/genpose2/depth', 10)
        self.pose_pub = self.create_publisher(PoseArray, '/genpose2/poses', 10)
        self.bridge = CvBridge()
        
    def publish_depth(self, depth_image, frame_idx):
        """Publish depth image as ROS2 Image message"""
        try:
            # Convert depth to uint16 format (millimeters)
            depth_msg = self.bridge.cv2_to_imgmsg(depth_image, encoding='16UC1')
            depth_msg.header.stamp = self.get_clock().now().to_msg()
            depth_msg.header.frame_id = 'camera_depth_optical_frame'
            self.depth_pub.publish(depth_msg)
            self.get_logger().info(f'Published depth image for frame {frame_idx}')
        except Exception as e:
            self.get_logger().error(f'Failed to publish depth: {str(e)}')
    
    def publish_poses(self, poses, lengths, frame_idx):
        """Publish object poses as ROS2 PoseArray message"""
        try:
            ...
            all_final_pose = poses[0].to(torch.float32).cpu().numpy()
            
            for obj_pose in all_final_pose:
                pose_msg = Pose()
                # obj_pose is 4x4 transformation matrix
                pose_msg.position.x = float(obj_pose[0, 3])
                pose_msg.position.y = float(obj_pose[1, 3])
                pose_msg.position.z = float(obj_pose[2, 3])
                
                # Convert rotation matrix to quaternion
                from scipy.spatial.transform import Rotation as R
                rot_matrix = obj_pose[:3, :3]
            ...
            self.get_logger().info(f'Published {len(pose_array_msg.poses)} poses for frame {frame_idx}')
        except Exception as e:
            self.get_logger().error(f'Failed to publish poses: {str(e)}')

def process_frame(frame, fds, genpose2, yolo, args, writer, frame_idx, depth_scale, K, baseline, ros_publisher=None):
    ...
    # run YOLO
    results = yolo.track(left_undistorted, persist=True, tracker="bytetrack.yaml", verbose=False)
    ...
    with torch.cuda.amp.autocast(True):
      # disp = fds.run_hierachical(img0, img1, iters=args.valid_iters, test_mode=True, small_ratio=0.5)
      disp = fds.forward(img0, img1, iters=args.valid_iters, test_mode=True)
    t1 = time.time()
    ...
    if ros_publisher is not None:
        depth_image_mm = (depth * depth_scale * 1000.0).astype(np.uint16)
        depth_image_mm[depth_image_mm == np.inf] = 0  # Remove invalid depths
        ros_publisher.publish_depth(depth_image_mm, frame_idx)
    ...
    if valid_output:
        ...
        # Publish poses via ROS2
        if ros_publisher is not None:
            ros_publisher.publish_poses(pose, length, frame_idx)
...
def main():
    ...
    # Initialize ROS2
    rclpy.init()
    ros_publisher = GenPose2Publisher()
    ...
    try:
        while True:
            ...
            if not ret:
                continue
            pose, length, infer_time = process_frame(frame, fds, genpose2, yolo, args, writer, frame_idx, depth_scale, K, baseline, ros_publisher)
            frame_idx += 1
