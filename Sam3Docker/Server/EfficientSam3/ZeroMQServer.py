...
SERVER_VERSION = "efficientsam3_v2"
...
def print_sam3_startup(version, config_path, host, port, visualize=True, vis_wait=0):
    print("=" * 70)
    print("EfficientSAM3 ZeroMQ Server")
    print("=" * 70)
...
def main():
    ...
    score_threshold = float(cfg["efficient_sam3"].get("score_threshold", 0.4))

    checkpoint_path = cfg["efficient_sam3"]["checkpoint_path"]
    text_encoder_type = cfg["efficient_sam3"].get("text_encoder_type", "MobileCLIP-S1")
    text_encoder_context_length = int(cfg["efficient_sam3"].get("text_encoder_context_length", 16))
    enable_segmentation = bool(cfg["efficient_sam3"].get("enable_segmentation", True))
    ...
    log_info("Loading EfficientSAM3 model, please wait...")
    ...
    processor = Sam3Processor(model, confidence_threshold=score_threshold)

    end_load_time = time.time()
    log_success(f"Model loaded successfully in {end_load_time - start_load_time:.2f} seconds")

    log_info("EfficientSAM3 runtime configuration:")
    log_kv("checkpoint_path", checkpoint_path)
    log_kv("text_encoder_type", text_encoder_type)
    log_kv("text_encoder_context_length", text_encoder_context_length)
    log_kv("score_threshold", score_threshold)
    log_kv("segmentation", enable_segmentation)
    ...
    while True:
        ...
        try:
            ...
            for prompt in prompts:
                ...
                current_masks = to_numpy(output["masks"])
                current_boxes = to_numpy(output["boxes"])
                current_scores = to_numpy(output["scores"])

                if current_masks.ndim == 4 and current_masks.shape[1] == 1:
